...
create or replace function public.get_monthly_gender_ranking_v2(
  p_gender text,
    ...
    select
      ...
      row_number() over (
        order by
          ...
          scores.presence_count desc,
          scores.first_checkins desc,
          lower(scores.full_name),
          scores.id
      )::bigint as position
...
revoke all on function public.get_monthly_gender_ranking_v2(
  text, integer, integer, integer, integer
) from public, anon;
grant execute on function public.get_monthly_gender_ranking_v2(
  text, integer, integer, integer, integer
